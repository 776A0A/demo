...
const functionalUtils = {
	...
	Stars: class {
		...
		add({ id, total = 5, image_1, image_2 }) {
			// 生成 stars
			this.total = total
			this.id = id
			if (!this.initImage) {
			...
			wrapper.addEventListener('click', e => {
				...
				if (ELEM.src === image_1) {
					...
					while ((currentElem = currentElem.previousSibling)) {
						currentElem.src = image_2
					}
					while ((_currentElem = _currentElem.nextSibling)) {
						_currentElem.src = image_1
					}
	...
	InputEmailHandler: class {
		...
		handleLi(value = '') {
			let html = '',
				_dataList = this.dataList
			let afterAt = value.split('@')[1]
			let x = false
			if (afterAt) {
				let regExp = new RegExp(`^${afterAt}`)
				if (!_dataList.every(item => !regExp.test(item))) {
					// dataList里的内容至少有一项匹配@后面的内容
					_dataList = []
			...
			;[...this.ul.querySelectorAll('li')][this.selectedLiIndex].classList.add(
				'active'
			)
		}
		...
		clickHandler(e) {
			...
			let isClickLi = [
				...document.querySelectorAll('#email-sug-wrapper li')
			].some(li => li === target)
			// 实体字符需解码
			isClickLi &&
